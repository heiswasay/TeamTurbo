...
import { WorkEntry, ItemChatMessage } from '../../types';
import { formatDateLabel } from '../../lib/dateUtils';
import { ItemFeedbackChat } from '../common/ItemFeedbackChat';
import { AlertCircle, Send, CheckCircle2, MessageSquare, CornerDownRight, ChevronDown, ChevronUp } from 'lucide-react';
      ...
      <div className="flex items-center gap-3.5 mb-5">
        ...
        <div>
          ...
          <p className="text-xs text-rose-200/80 mt-0.5">
            The team lead has requested adjustments on the following tasks. Please review remarks, chat directly with lead, and submit a follow-up note.
          </p>
        </div>
      </div>
          ...
          return (
            ...
            >
              ...
              <div className="flex items-center justify-between pt-1">
                <button
                  type="button"
                  onClick={() => setActiveChatEntryId(activeChatEntryId === entry.id ? null : entry.id)}
                  className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-semibold transition-all ${
                    activeChatEntryId === entry.id
                      ? 'bg-indigo-600 text-white shadow-md shadow-indigo-600/20'
                      : entryChatMessages.length > 0
                      ? 'bg-indigo-600/20 hover:bg-indigo-600/30 text-indigo-300 border border-indigo-500/30'
                      : 'bg-[#161B27] hover:bg-slate-700/50 text-slate-300 border border-slate-700/60'
                  }`}
                >
                  <MessageSquare className="w-3.5 h-3.5 text-indigo-400" />
                  <span>
                    {entryChatMessages.length > 0
                      ? `Chat with Lead (${entryChatMessages.length})`
                      : 'Discuss / Chat with Lead'}
                  </span>
                  {activeChatEntryId === entry.id ? (
              ...
              {/* Inline Chat for Rework Entry */}
              {activeChatEntryId === entry.id && onSendMessage && (
                <ItemFeedbackChat
                  targetId={entry.id}
                  targetType="work_entry"
                  targetTitle={`Rework: ${entry.company}`}
